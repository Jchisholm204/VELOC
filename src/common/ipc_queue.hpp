...
template <typename T> struct client_queue_t {
    ...
    client_queue_t(const T_allocator& alloc)
        : pending(alloc), progress(alloc) {}
};
...
template <typename T> class comm_client_t {
  ...
  public:
    comm_client_t(int id)
        : segment(open_or_create, IPC_BUFFER.c_str(), IPC_MAX_SIZE),
        ...
        data =
            segment.find_or_construct<container_t>(std::to_string(id).c_str())(
                segment.get_allocator<typename container_t::T_allocator>());
    }
...
template <typename T> class comm_backend_t {
    ...
    container_t* find_non_empty_pending() {
        for (managed_shared_memory::const_named_iterator it =
                 segment.named_begin();
             it != segment.named_end(); ++it) {

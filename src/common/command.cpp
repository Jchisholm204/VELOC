...
command_t::command_t(int r, int c, int v, const std::string& src)
    : unique_id(r), command(c), version(v) {
    if (src.length() + 1 > CKPT_NAME_MAX)
        FATAL("checkpoint name '" + src + "' is longer than admissible size " +
              std::to_string(CKPT_NAME_MAX));
    std::strcpy(name, src.c_str());
}

void command_t::assign_path(const std::string& src) {
    if (src.length() + 1 > PATH_MAX)
        FATAL("routed path '" + src + "' is longer than admissible size " +
...
bool command_t::match(const std::string& str, const std::regex& ex, int& id,
                      int& version) {
    std::smatch sm;
...
std::string command_t::agg_filename(const std::string& prefix) const {
    return prefix + "/" + std::string(name) + "-agg-" +
           std::to_string(version) + ".dat";
}

...
class config_t {
  ...
  public:
    ...
    template <class T>
    bool get_optional(const std::string& param, T& value) const;
    std::string get(const std::string& param) const;
    bool get_bool(const std::string& param, bool def) const;

    bool is_sync() const { return sync_mode; }
    const std::string& get_cfg_file() const { return cfg_file; }
    storage_module_t* storage() const { return sm; }
};

...
config_t::config_t(const std::string& f, bool is_backend) : cfg_file(f) {
    ...
    if (!cfg_file.empty()) {
        ...
        if (reader->ParseError() > 0)
            FATAL("error parsing config file " << cfg_file << " at line "
                                               << reader->ParseError());
    }
    ...
    if (!get_optional("mode", val) || (val != "sync" && val != "async"))
        FATAL("mode of operation " << val
                                   << " is invalid, must be sync/async!");
    sync_mode = (val == "sync");
    ...
    if (get_optional("daos_pool", persistent) &&
        get_optional("daos_cont", val)) {
        if constexpr (!std::is_same<daos_module_t, storage_module_t>::value) {
            INFO("using DAOS to interact with persistent storage, "
                 "pool/container: "
                 << persistent << "/" << val);
            sm = new daos_module_t(scratch, persistent, val);
        } else
            FATAL("DAOS requested but not available at compile time, please "
                  "link with DAOS");
    } else if (get_optional("persistent", persistent)) {
        if (get_optional("axl_type", val)) {
            if constexpr (!std::is_same<axl_module_t,
                                        storage_module_t>::value) {
                INFO("using AXL to interact with persistent storage, AXL type: "
                     << val);
                sm = new axl_module_t(scratch, persistent, val);
            } else
                FATAL("AXL requested but not available at compile time, please "
                      "link with AXL");
        } else {
            if (get_bool("aggregated", false)) {
                INFO("using POSIX to interact with persistent storage in "
                     "aggregated file mode, path: "
                     << persistent);
                std::string meta;
...
// use explicit template specializations to restrict the data types accepted by
// the parser
template bool config_t::get_optional<std::string>(const std::string& param,
                                                  std::string& value) const;
template bool config_t::get_optional<int>(const std::string& param,
                                          int& value) const;
template bool config_t::get_optional<unsigned int>(const std::string& param,
                                                   unsigned int& value) const;

template <class T>
bool config_t::get_optional(const std::string& param, T& value) const {

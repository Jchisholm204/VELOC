...
bool parse_dir(const std::string& p, const std::string& cname,
               dir_callback_t f) {
    DIR* dir;
    dir = opendir(p.c_str());
    if (dir == NULL)
...
bool write_file(const std::string& source, unsigned char* buffer,
                ssize_t size) {
    bool ret;
    ...
    if (!ret)
        ERROR("cannot write " << size << " bytes to " << source
...
bool read_file(const std::string& source, unsigned char* buffer, ssize_t size) {
    ...
    if (!ret)
        ERROR("cannot read " << size << " bytes from " << source
                             << " , error = " << std::strerror(errno));
    close(fd);
    return ret;
}
...
bool file_transfer_loop(int fs, size_t soff, int fd, size_t doff,
                        size_t remaining) {
    bool success = true;
    while (remaining > 0) {
        ssize_t transferred = copy_file_range(fs, (off64_t*) &soff, fd,
                                              (off64_t*) &doff, remaining, 0);
        if (transferred == -1) {
            success = false;
            break;
        }
...
bool file_transfer_loop(int fs, size_t soff, int fd, size_t doff,
    ...
    while (remaining > 0) {
        ssize_t transferred =
            pread(fs, buff, std::min(MAX_BUFF_SIZE, (size_t) remaining), soff);
        if (transferred == -1 ||
            pwrite(fd, buff, transferred, doff) != transferred) {
            success = false;
            break;
        }
        remaining -= transferred;
...
bool posix_transfer_file(const std::string& source, const std::string& dest,
                         size_t soffset, size_t doffset, size_t size) {
    ...
    if (success) {
        ...
        ERROR("cannot copy " << source << " to " << dest
                             << "; error = " << std::strerror(errno));
    return success;
}

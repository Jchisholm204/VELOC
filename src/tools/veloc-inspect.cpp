...
void exit_with_usage() {
    std::cerr << "Usage: veloc-inspect --header | --extract <id> [--size "
                 "<size>] --ckpt <ckpt_name>"
              << std::endl;
    std::cerr << "Note: shortcuts (-h, -e, -s, -c) are also allowed"
              << std::endl;
    std::cerr << "Extract will dump binary output to stdout. You probably want "
...
int main(int argc, char** argv) {
    ...
    static struct option long_ops[] = {{"header", no_argument, 0, 'h'},
    ...
    if (header) {
        ...
        for (const auto& e : region_info) {
            std::cout << "id = " << e.first << ", size = " << e.second
                      << std::endl;
            total += e.second;
        }
    ...
    try {
        ...
        if (!found)
            throw std::ifstream::failure("cannot find region " +
                                         std::to_string(id) + " in the header");
        if (region_info[id] < size)
            throw std::ifstream::failure(
                "region " + std::to_string(id) + " is of size " +
                std::to_string(region_info[id]) +
                ", which is smaller than requested size " +
                std::to_string(size));
        f.seekg(seek_offset);
        if (size == 0)
            size = region_info[id];
        ...
        ERROR("cannot read from checkpoint file " << ckpt_name
                                                  << ", reason: " << e.what());
        return -1;
    }

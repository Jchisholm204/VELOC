...
struct async_command_t {
    ...
    async_command_t(int _op, int _fl, int _fr) : op(_op), fl(_fl), fr(_fr) {}
...
static void async_write() {
    ...
    while (true) {
        ...
        while (!static_context.finished &&
               static_context.async_op_queue.size() == 0)
        ...
        if (cmd.op == async_command_t::CLOSE) {
            ...
            DBG("async write, fl = " << cmd.fl << ", fr = " << cmd.fr
                                     << ", offset = " << cmd.offset
                                     << ", size = " << cmd.size);
            bool result = file_transfer_loop(cmd.fl, cmd.offset, cmd.fr,
                                             cmd.offset, cmd.size);
            if (!result)
            ...
            FATAL("internal async thread error: opcode " << cmd.op
                                                         << " not recognized");
...
static void send_command(const async_command_t& cmd) {
    std::unique_lock<std::mutex> lock(static_context.async_mutex);
    while (!static_context.finished && static_context.async_op_size + cmd.size >
                                           static_context.MAX_QUEUE_SIZE)
        static_context.async_cond.wait(lock);
    if (static_context.finished)
        return;
...
posix_cached_file_t::posix_cached_file_t(const std::string& scratch_path)
...
bool posix_cached_file_t::open(const std::string& fname, int flags,
                               mode_t mode) {
    if (fl != -1 || fr != -1) {
        ERROR("cannot open file " << fname
                                  << ", reason: another file already opened");
        return false;
    }
    std::string local_fname =
        scratch + "/" + std::string(basename(fname.c_str()));
    fl = ::open(local_fname.c_str(), O_RDWR | O_CREAT, mode);
    if (fl == -1) {
        ERROR("cannot open local file " << fname
                                        << ", reason: " << strerror(errno));
        return false;
    }
    fr = ::open(fname.c_str(), flags, mode);
    if (fr == -1) {
        ERROR("cannot open remote file " << fname
                                         << ", reason: " << strerror(errno));
        return false;
    }
    return true;
...
bool posix_cached_file_t::pread(void* buf, size_t count, off_t offset) {
    ...
    while (count > 0) {
        ...
        if (br <= 0) {
            ERROR("reading " << count << " bytes from offset " << offset
...
bool posix_cached_file_t::pwrite(const void* buf, size_t size, off_t offset) {
    ...
    while (count > 0) {
        ...
        if (br == -1) {
            ERROR("writing " << count << " bytes from offset " << off
                             << " failed, reason: " << strerror(errno));
            return false;
        }
        count -= br;
...
// PIMPL wrappers
namespace veloc {
cached_file_t::cached_file_t(const std::string& scratch_path)
    : ptr(new ::posix_cached_file_t(scratch_path)) {
}

...
class client_impl_t : public veloc::client_t {
    struct region_t {
        ...
        region_t(const serializer_t& _s, const deserializer_t& _d)
            : ptr(NULL), s(_s), d(_d) {}
    };
    ...
    config_t cfg;
    MPI_Comm comm = MPI_COMM_NULL, local = MPI_COMM_NULL,
             backends = MPI_COMM_NULL;
    int rank, no_ranks;
  ...
  public:
    ...
    virtual bool mem_protect(int id, const serializer_t& s,
                             const deserializer_t& d, const std::string& name);

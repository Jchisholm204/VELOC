...
#define BOOL_CALL(x)                                                           \
    (veloc_client != NULL && (x)) ? VELOC_SUCCESS : VELOC_FAILURE;
#define VOID_CALL(x) veloc_client != NULL ? (x, VELOC_SUCCESS) : VELOC_FAILURE;
#define INT_CALL(x) veloc_client != NULL ? x : VELOC_FAILURE;

extern "C" int VELOC_Mem_protect(int id, void* ptr, size_t count,
                                 size_t base_size) {
    return BOOL_CALL(veloc_client->mem_protect(id, ptr, count, base_size));
}

extern "C" int VELOC_Region_protect(int id, void* ptr, size_t count,
                                    size_t base_size, const char* name) {
    return BOOL_CALL(
...
veloc::client_t* veloc::get_client(unsigned int id,

...
bool client_impl_t::mem_protect(int id, void* ptr, size_t count,
                                size_t base_size, const std::string& name) {
    return mem_regions[name]
        .insert_or_assign(id, region_t(ptr, count * base_size))
        .second;
}

bool client_impl_t::mem_protect(int id, const serializer_t& s,
                                const deserializer_t& d,
...
bool client_impl_t::checkpoint_end(bool /*success*/) {
    if (aggregated) {
        long offset = 0,
             ckpt_size = file_size(current_ckpt.filename(cfg.get("scratch")));
...
int client_impl_t::restart_test(const std::string& name, int needed_version,
    ...
    int version =
        run_blocking(command_t(check_rank(target_rank), command_t::TEST,
                               needed_version, name.c_str()));
...
bool client_impl_t::restart(const std::string& name, int version,
    ...
    return restart_begin(name, version, target_rank) &&
           recover_mem(VELOC_CKPT_ALL, {}) && restart_end(true);
}
...
bool client_impl_t::restart_begin(const std::string& name, int version,
    ...
    current_ckpt = command_t(check_rank(target_rank), command_t::RESTART,
                             version, name.c_str());

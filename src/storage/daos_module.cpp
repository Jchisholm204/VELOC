...
daos_module_t::daos_module_t(const std::string& s, const std::string& p,
                             const std::string& c)
    : scratch(s) {
    if (daos_init() != 0)
...
bool daos_module_t::flush(const command_t& cmd) {
    ...
    if (rc) {
        ERROR("cannot open DAOS object id (" << oid.lo << ", " << oid.hi
                                             << "); error = " << rc);
        return false;
    }
    ...
    if (fi == -1 || size == -1) {
        ERROR("cannot open source " << source
                                    << "; error = " << std::strerror(errno));
        daos_kv_close(oh, NULL);
        return false;
    }
    unsigned char* buff =
        (unsigned char*) mmap(NULL, size, PROT_READ, MAP_PRIVATE, fi, 0);
    if (buff == MAP_FAILED) {
        close(fi);
        daos_kv_close(oh, NULL);
        ERROR("cannot mmap source " << source
                                    << "; error = " << std::strerror(errno));
        return false;
    }
    close(fi);
    ...
    TIMER_STOP(io_timer, "transferred " << source << " to DAOS object id ("
                                        << oid.lo << ", " << oid.hi << ")");
    return true;
}
...
bool daos_module_t::restore(const command_t& cmd) {
    ...
    size_t size;
    rc =
        daos_kv_get(oh, DAOS_TX_NONE, 0, cmd.stem().c_str(), &size, NULL, NULL);
    if (rc) {
        daos_kv_close(oh, NULL);
        ERROR("cannot get size of key " << cmd.stem()
                                        << "; error code = " << rc);
        return false;
    }
    ...
    if (posix_fallocate(fo, 0, size)) {
        close(fo);
        daos_kv_close(oh, NULL);
        ERROR("cannot preallocate " << dest
                                    << "; error = " << std::strerror(errno));
        return false;
    }
    unsigned char* buff =
        (unsigned char*) mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_SHARED,
                              fo, 0);
    if (buff == MAP_FAILED) {
        close(fo);
        daos_kv_close(oh, NULL);
        ERROR("cannot mmap destination "
              << dest << "; error = " << std::strerror(errno));
        return false;
    }
    close(fo);
    rc =
        daos_kv_get(oh, DAOS_TX_NONE, 0, cmd.stem().c_str(), &size, buff, NULL);
    munmap(buff, size);
    daos_kv_close(oh, NULL);
    if (rc) {

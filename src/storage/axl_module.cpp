...
axl_module_t::axl_module_t(const std::string& s, const std::string& p,
                           const std::string& axl_type_str)
    : posix_module_t(s, p) {
    auto e = axl_type_strs.find(axl_type_str);
    if (e == axl_type_strs.end())
        FATAL("AXL has no transfer type called \""
              << axl_type_str << "\", please consult the documentation");
    axl_type = e->second;
    int ret = AXL_Init();
    if (ret)
...
bool axl_module_t::axl_transfer_file(const std::string& source,
    ...
    return true;
err:
    ERROR("AXL transfer from " << source << " to " << dest
...
bool axl_module_t::flush(const command_t& cmd) {
    // memory-based API
    if (cmd.original[0] == 0)
        return axl_transfer_file(cmd.filename(scratch),
                                 cmd.filename(persistent));
    // file-based API
    ...
    if (symlink(cmd.original, cmd.filename(persistent).c_str())) {
        ERROR("cannot create symlink " << cmd.filename(persistent)
                                       << " pointing at " << cmd.original

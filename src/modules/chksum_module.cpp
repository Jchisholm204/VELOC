...
chksum_module_t::chksum_module_t(const config_t& c) : cfg(c) {
    ...
    if (active && !check_dir(cfg.get("meta"))) {
        ERROR("metadata directory "
              << cfg.get("meta") << " inaccessible, checksumming deactivated!");
        active = false;
    }
...
static bool chksum_file(const std::string& source, unsigned char* result) {
    ...
    unsigned char* buff =
        (unsigned char*) mmap(NULL, size, PROT_READ, MAP_PRIVATE, fd, 0);
...
int chksum_module_t::process_command(const command_t& c) {
    ...
    std::string meta = c.meta_filename(cfg.get("meta")),
    ...
    case command_t::CHECKPOINT:
        ...
        return write_file(meta, chksum, HASH_SIZE) ? VELOC_SUCCESS
                                                   : VELOC_FAILURE;
    ...
    case command_t::RESTART:
        ...
        if (match)
            ...
            ERROR("checksum of file "
                  << local << " does not match records, restart will fail");
            return VELOC_FAILURE;
        }

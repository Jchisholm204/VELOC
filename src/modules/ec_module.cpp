...
ec_module_t::ec_module_t(const config_t& c, MPI_Comm cm) : cfg(c), comm(cm) {
    if (!cfg.get_optional("ec_interval", interval)) {
        INFO("EC interval not specified, every checkpoint will be protected "
             "using EC");
        interval = 0;
    }
    int rank, ranks;
    ...
    if (ER_Init(cfg.get_cfg_file().c_str()) != ER_SUCCESS)
        FATAL(
            "Failed to initialize ER from config file: " << cfg.get_cfg_file());
...
int ec_module_t::process_commands(const std::vector<command_t>& cmds) {
    ...
    std::string name = cfg.get("scratch") + "/" + cmds[0].name + "-ec-" +
    ...
    if (command == command_t::CHECKPOINT) {
        ...
        int set_id = ER_Create(comm, comm_domain, name.c_str(),
                               ER_DIRECTION_ENCODE, scheme_id);
        if (set_id == -1) {
        ...
        if (result) {
            INFO("skipping ER rebuild, local checkpoints available for: "
                 << name);
            return VELOC_SUCCESS;
        }
        ...
        int set_id =
            ER_Create(comm, comm_domain, name.c_str(), ER_DIRECTION_REBUILD, 0);
        if (set_id == -1) {
            ERROR("ER_Create failed for checkpoint: " << name);
            return VELOC_FAILURE;
        }

...
client_aggregator_t::client_aggregator_t(const config_t& cfg,
                                         const agg_function_t& f,
                                         const single_function_t& g)
...
int client_aggregator_t::process_command(const command_t& c) {
    if (c.command == command_t::INIT) {
        ...
        return single_function(c);
    } else if (c.command == command_t::CHECKPOINT ||
               c.command == command_t::RESTART) {
        if (max_parallelism < client_set.size()) {
            ERROR("max_parallelism is too low, needs to be higher than "
                  << client_set.size() << " to activate aggregated commands");

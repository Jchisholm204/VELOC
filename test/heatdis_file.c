...
int main(int argc, char* argv[]) {
    ...
    if (v > 0) {
        ...
        if (fd != NULL) {
            ...
            if (fread(h, sizeof(double), M * nbLines, fd) != M * nbLines) {
                valid = 0;
            }
            if (fread(g, sizeof(double), M * nbLines, fd) != M * nbLines) {
                valid = 0;
            }
            fclose(fd);
        } else
    ...
    while (i < ITER_TIMES) {
        ...
        if (i % CKPT_FREQ == 0) {
            ...
            if (fd != NULL) {
                ...
                if (fwrite(h, sizeof(double), M * nbLines, fd) != M * nbLines) {
                    valid = 0;
                }
                if (fwrite(g, sizeof(double), M * nbLines, fd) != M * nbLines) {
                    valid = 0;
                }
                fclose(fd);
            } else
